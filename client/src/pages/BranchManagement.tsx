...
const BranchManagement = () => {
  ...
  const fetchBranches = async () => {
    try {
      const res = await axiosInstance.get('https://unicorn-cafe-management-x6vf.vercel.app/api/branches', {
    ...
    try {
      ...
      if (editingId) {
        response = await axiosInstance.put(
          `https://unicorn-cafe-management-x6vf.vercel.app/api/branches/${editingId}`,
          payload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        response = await axiosInstance.post(
          'https://unicorn-cafe-management-x6vf.vercel.app/api/branches',
          payload,
          { headers: { Authorization: `Bearer ${token}` } }
        );
  ...
  const handleDelete = async (id?: string) => {
    ...
    try {
      await axiosInstance.delete(`https://unicorn-cafe-management-x6vf.vercel.app/api/branches/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchBranches();
